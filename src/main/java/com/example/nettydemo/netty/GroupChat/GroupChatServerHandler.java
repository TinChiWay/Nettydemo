...
public class GroupChatServerHandler extends SimpleChannelInboundHandler<String> {
    ...
    public void handlerRemoved(ChannelHandlerContext ctx) throws Exception {
        // 已经执行了channelGroup.remove(ctx.channel());
        channelGroup.writeAndFlush("[客户端]" + ctx.channel().remoteAddress() + "离开了" + sdf.format(new Date()) + "\n");
    }
    ...
    protected void channelRead0(ChannelHandlerContext ctx, String msg) throws Exception {
        ...
        channelGroup.forEach(ch -> {
            if (ch != channel) {
                // 不是当前的channel,转发消息
                ch.writeAndFlush("[成员" + channel.remoteAddress() + "]:" + msg + " [" + sdf.format(new Date()) + "]\n");
            } else {
                ch.writeAndFlush("[自己]:" + msg + " [" + sdf.format(new java.util.Date()) + "]\n");
            }
        });
    }
